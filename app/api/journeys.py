...
def create_journey(payload: JourneyCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    contact_name = None
    if payload.trusted_contact_id:
        contact = db.get(TrustedContact, payload.trusted_contact_id)
        if contact and contact.owner_id == current_user.id:
            contact_name = contact.name
    journey = Journey(owner_id=current_user.id, trusted_contact_name=contact_name, **payload.model_dump())
    db.add(journey)
    db.commit()
    db.refresh(journey)
    return journey
...
async def start_journey(journey_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    contact = db.get(TrustedContact, journey.trusted_contact_id) if journey.trusted_contact_id else None
    if contact:
        await notify_journey_contact(current_user.full_name, contact, "started", journey.from_label, journey.to_label)
    return journey
...
async def update_location(journey_id: str, payload: JourneyLocationUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if journey.status == JourneyStatus.deviated and not was_deviated and journey.notify_on_deviation:
        contact = db.get(TrustedContact, journey.trusted_contact_id) if journey.trusted_contact_id else None
        if contact:
            await notify_journey_contact(current_user.full_name, contact, "deviated", journey.from_label, journey.to_label)
    return journey
...
async def end_journey(journey_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    contact = db.get(TrustedContact, journey.trusted_contact_id) if journey.trusted_contact_id else None
    if contact:
        await notify_journey_contact(current_user.full_name, contact, "arrived", journey.from_label, journey.to_label)
    return journey
